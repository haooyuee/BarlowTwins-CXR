...
class DetCAMVisualizer:
    ...
    def _draw_boxes(self, boxes, labels, image):
        ...
        for i, box in enumerate(boxes):
            ...
            # 获取文本尺寸
            text_width, text_height = cv2.getTextSize(self.classes[label].upper(), cv2.FONT_HERSHEY_SIMPLEX, 0.3, 1)[0]
            ...
            # 绘制实心边界框和文本
            cv2.rectangle(output, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), color, thickness)
            cv2.putText(output, self.classes[label].upper(), (int(box[0]), int(box[1])-5), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1, cv2.LINE_AA)
