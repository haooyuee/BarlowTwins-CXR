...
def main():
    ...
    for image_path in images:
        ...
        for i, (bbox, score, label) in enumerate(zip(bboxes, scores, labels)):
            ...
            single_bbox = np.array([bbox])
            single_label = np.array([label])
            targets = [DetBoxScoreTarget(bboxes=single_bbox, labels=single_label)]
            print(i)
            print(single_bbox)
            print(single_label)
            model.set_input_data(image)
            if args.method in GRAD_BASE_METHOD_MAP:
                ...
                model.set_input_data(image, bboxes=single_bbox, labels=single_label)
            ...
            image_with_bounding_boxes = det_cam_visualizer.show_cam(
                                image, single_bbox, single_label, grayscale_cam, not args.no_norm_in_bbox)

            if args.out_dir:
                mmcv.mkdir_or_exist(args.out_dir)
                out_file = os.path.join(args.out_dir, f"{os.path.basename(image_path).split('.')[0]}_bbox{i}.png")
